...
def list_vpcs(vpc_name: str, gcp_client: GCP_Crds.get_gcp_crds = None):
    ...
    if vpc_name == "" or vpc_name == None:
        ...
        return API_Response(message="VPCs fetched successfully", data=vpc, status_code=200).model_dump()
...
def create_vpc_subnetwork_firewall(details: VPC_Schema, gcp_client: GCP_Crds.get_gcp_crds = None):
    ...
    if not VPC.check_vpc_exists(gcp_crds=gcp_client, vpc_name=details.name, vpc_client=vpc_client):
        vpc = create_vpc(gcp_client=gcp_client, details=details, vpc_client=vpc_client)
        if type(vpc) == API_Response:
            return vpc.model_dump()
        if vpc is None:
            return API_Response(error= "VPC creation failed", status_code=400).model_dump()
    else:
        vpc = VPC.get_vpc(gcp_crds=gcp_client, vpc_name=details.name, vpc_client=vpc_client)
    ...
    if details.autoCreateSubnetworks != True: 
        ...
        subnetwork = create_subnetwork(gcp_client=gcp_client, details=jsonable_encoder(subnetwork_details),vpc_client=vpc_client)
        if type(subnetwork) == API_Response:
            return subnetwork.model_dump()
        if subnetwork is None:
            return API_Response(error="VPC Created butSubnetwork creation failed", status_code=400).model_dump()
        vpc["subnetworks"] = [subnetwork]

    firewall_rules = get_sample_firewalls(vpc_name=details.name, vpc_target_link=vpc["targetLink"], subnetwork_ip=subnetwork_details.ipCidrRange)
    firewalls = create_firewall(gcp_client=gcp_client, firewall_rules=firewall_rules, vpc_client=vpc_client)
    # if type(firewalls) == API_Response:
    #     return firewalls.model_dump()
    if firewalls is None or len(firewalls) == 0:
        return API_Response(error="VPC Created but Firewall creation failed", status_code=400).model_dump()
    vpc["firewall"] = firewalls
    return API_Response(message="VPC created successfully", data=vpc, status_code=200).model_dump()
...
def create_firewall(firewall_rules: list[Firewall_Schema], gcp_client: GCPSrvAcc.Credentials = None, vpc_client: Any = None):
    ...
    for firewall in firewall_rules:
        if VPC.check_firewall_exists(gcp_crds=gcp_client, firewall_name=firewall["name"], vpc_client=vpc_client):
            continue
        firewall = VPC.create_firewall(gcp_crds=gcp_client, details=firewall, vpc_client=vpc_client)
...
def create_subnetwork(details: SubNetwork_Schema, gcp_client: GCPSrvAcc.Credentials = None, vpc_client: Any = None):
    ...
    if VPC.check_subnetwork_exists(gcp_crds=gcp_client, subnetwork_name=details_json["name"], vpc_client=vpc_client):
        return API_Response(message="Subnetwork already exists", status_code=400)
    subnetwork = VPC.create_subnetwork(gcp_crds=gcp_client, details=details_json, vpc_client=vpc_client)
    return subnetwork
...
def create_vpc(details: VPC_Schema, gcp_client: GCPSrvAcc.Credentials = None, vpc_client: Any = None):
    ...
    if VPC.check_vpc_exists(gcp_crds=gcp_client, vpc_name=details_json["name"], vpc_client=vpc_client):
        return API_Response(message="VPC already exists", status_code=400)
...
def delete_vpc(vpc_name: str, gcp_client: GCP_Crds.get_gcp_crds = None):
    ...
    if type(vpc) == API_Response:
        return vpc.model_dump()
    if not vpc:
        return API_Response(message="VPC not found", status_code=400).model_dump()
    firewalls = VPC.list_vpc_firewalls(gcp_crds=gcp_client, vpc_target_link=vpc["selfLink"], vpc_client=vpc_client)
    if len(firewalls.get("items", [])):
        for firewall in firewalls.get("items", []):
            VPC.delete_firewall(firewall_name=firewall["name"], gcp_crds=gcp_client, vpc_client=vpc_client)
        time.sleep(10)
    subnetworks = VPC.list_vpc_subnetworks(gcp_crds=gcp_client, vpc_client=vpc_client, vpc_target_link=vpc["selfLink"], region=settings.GCP_Config.DEFAULT_REGION)
    ...
    time.sleep(30)
    return API_Response(message="VPC deleted successfully", status_code=200).model_dump()

def get_sample_firewalls(vpc_name: str, vpc_target_link: str, subnetwork_ip: str):
    return [
        {
        ...
        },
        {
            ...
            "allowed": [
                {
                    "IPProtocol": "tcp",
                    "ports":  ["22", "80", "443", "3000", "4000", "4001", "4002", "4003", "5000", "5001","5002","5003","6000","6001", "6002"],
                }
            ],
            "targetTags": ["https-server", vpc_name],
            "description": "Allow SSH, HTTP, HTTPS, 3000, 4000, 4001, 4002, 4003, 5000, 5001, 5002, 5003, 6000, 6001, 6002",
        },
        {
