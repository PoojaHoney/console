...
def create_instance(details: Instance):
    ...
    try:
        ...
        #delete instance - delete all resources created
        return API_Response(error=exp, status_code=400).model_dump()

def delete_instance(instance_id: str, name: str, gcp_client: GCP_Crds.get_gcp_crds = None):
    if gcp_client == None:
        gcp_client = GCP_Crds.get_gcp_crds({"project_id":settings.GCP_Config.PROJECT_ID,
                                "private_key_id":settings.GCP_Config.SRV_ACC_PRIVATE_KEY_ID,
                                "private_key":settings.GCP_Config.SRV_ACC_PRIVATE_KEY,
    ...
    #delete iam role
    IAM.delete_service_account(service_account=f"{instance_id}@{settings.GCP_Config.PROJECT_ID}.iam.gserviceaccount.com", gcp_crds=gcp_client)
    #delete vpc
    VPC.delete_vpc(vpc_name=f"{instance_id}{name}", gcp_client=gcp_client)
    #delete compute engine
    # Compute.delete_instance(instance_name=f"{instance_id}{name}", gcp_client=gcp_client)