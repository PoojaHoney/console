...
def get_vpc_client(gcp_crds):
    # if gcp_crds == None:
    #     return build(serviceName="compute", version="v1")
    # else:
    return build(serviceName="compute", version="v1", credentials=gcp_crds)
    # return compute_v1.NetworksClient(credentials=gcp_crds)
...
def list_vpcs(gcp_crds: GCPSrvAcc.Credentials, vpc_client: Any = None):
    ...
    vpcs_result = vpc_client.networks().list(project=project_id).execute()
    return vpcs_result["items"]
...
def get_vpc(gcp_crds: GCPSrvAcc.Credentials, vpc_name: str, vpc_client: Any = None):
    ...
    project_id = gcp_crds.project_id
    try:
        result = vpc_client.networks().get(project=project_id, network=vpc_name).execute()
    except Exception as exp:
        error_data = json.loads(exp.content.decode("utf-8"))
        if "was not found" in error_data['error']['message']:
            return None
        return error_data
    return result
...
def create_subnetwork(gcp_crds: GCPSrvAcc.Credentials, details: dict, vpc_client: Any = None):
    if vpc_client is None:
        vpc_client = get_vpc_client(gcp_crds)
    response = vpc_client.subnetworks().insert(project=gcp_crds.project_id,
                                               region=details["region"], body=details).execute()
    time.sleep(10)
    return response
...
def get_subnetwork(gcp_crds: GCPSrvAcc.Credentials, subnetwork_name: str, vpc_client: Any = None):
    ...
    project_id = gcp_crds.project_id
    try:
        subnetwork = vpc_client.subnetworks().get(
            project=project_id, subnetwork=subnetwork_name).execute()
    except:
        subnetwork = None
    return subnetwork
...
def list_vpc_subnetworks(gcp_crds: GCPSrvAcc.Credentials, vpc_target_link: str, region: str, vpc_client: Any = None):
    if vpc_client is None:
        vpc_client = get_vpc_client(gcp_crds)
    project_id = gcp_crds.project_id
    subnetworks = vpc_client.subnetworks().list(project=project_id,
                                                filter=f'network="{vpc_target_link}"', region=region).execute()
    return subnetworks
...
def list_vpc_firewalls(gcp_crds: GCPSrvAcc.Credentials, vpc_target_link: str, vpc_client: Any = None):
    if vpc_client is None:
        vpc_client = get_vpc_client(gcp_crds)
    project_id = gcp_crds.project_id
    firewalls = vpc_client.firewalls().list(
        project=project_id, filter=f'network="{vpc_target_link}"').execute()
    return firewalls
...
def get_firewall(gcp_crds: GCPSrvAcc.Credentials, firewall_name: str, vpc_client: Any = None):
    ...
    project_id = gcp_crds.project_id
    try:
        firewall = vpc_client.firewalls().get(
...
def create_firewall(gcp_crds: GCPSrvAcc.Credentials, details: dict, vpc_client: Any = None):
    if vpc_client is None:
        vpc_client = get_vpc_client(gcp_crds)
    project_id = gcp_crds.project_id
    firewall = vpc_client.firewalls().insert(
        project=project_id, body=details).execute()
    return firewall
...
def delete_vpc(gcp_crds: GCPSrvAcc.Credentials, vpc_name: str, vpc_client: Any = None):
    if vpc_client is None:
        vpc_client = get_vpc_client(gcp_crds)
    response = vpc_client.networks().delete(
        project=gcp_crds.project_id, network=f"{vpc_name}").execute()
    return response
...
def delete_firewall(gcp_crds: GCPSrvAcc.Credentials, firewall_name: str, vpc_client: Any = None):
    if vpc_client is None:
        vpc_client = get_vpc_client(gcp_crds)
    response = vpc_client.firewalls().delete(project=gcp_crds.project_id,
...
def delete_subnetwork(gcp_crds: GCPSrvAcc.Credentials, subnetwork_name: str, region: str, vpc_client: Any = None):
    if vpc_client is None:
        vpc_client = get_vpc_client(gcp_crds)
    response = vpc_client.subnetworks().delete(project=gcp_crds.project_id,
                                               region=region, subnetwork=subnetwork_name).execute()
    return response
