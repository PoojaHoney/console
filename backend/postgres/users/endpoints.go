...
func (srv *Service) Get(ctx *fiber.Ctx) error {
	...
	var preparedFilters map[string]interface{}
	...
	if filtersStr != "" {
		...
		preparedFilters = preparePostgresFilters(filters)
	}
	users, err := getUsers(srv.PostgresDB, preparedFilters)
	if err != nil {
		return ctx.Status(fiber.StatusBadRequest).JSON(fiber.Map{
...
func (srv *Service) SendOTPVerificationMail(ctx *fiber.Ctx) error {
	...
	user, err := getUser(srv.PostgresDB, map[string]interface{}{"email": email.Email})
	if err != nil {
		return ctx.Status(fiber.StatusBadRequest).JSON(fiber.Map{
...
func ValidateToken(srv *Service) func(*fiber.Ctx) error {
	return func(ctx *fiber.Ctx) error {
		...
		if user["userId"].(uuid.UUID).String() != "" || user["email"].(string) != "" {
			ctx.Context().SetUserValue("userId", user["userId"].(uuid.UUID).String())
